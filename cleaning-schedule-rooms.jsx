...
const [historySubView, setHistorySubView] = useState(“period”);
...
let existing = {};
try {
const r = await localStorageGet(sk);
if (r?.value) existing = JSON.parse(r.value);
} catch (*) {}
await localStorageSet(sk, JSON.stringify({ …existing, …data }));
const idxResult = await localStorageGet(“archive:index”).catch(() => null);
const index = idxResult?.value ? JSON.parse(idxResult.value) : [];
if (!index.includes(periodKey)) {
index.push(periodKey);
await localStorageSet(“archive:index”, JSON.stringify(index));
}
} catch (*) {}
}, []);

// ── Prune expired completions ────────────────────────────────────────────
...
async function load() {
try {
const result = await localStorageGet(“completions”);
if (result?.value) {
...
useEffect(() => {
if (loading) return;
localStorageSet(“completions”, JSON.stringify(completions)).catch(() => {});
}, [completions, loading]);
...
try {
const r = await localStorageGet(“customTasks”);
if (r?.value) {
...
localStorageSet(“customTasks”, JSON.stringify(customTasks)).catch(e => console.error(“SAVE customTasks error:”, e));
...
return customTasks[roomId][freq];
}
...
try {
const r = await localStorageGet(“archive:index”);
              ...
              <div onClick={() => tasks.length > 0 && toggleRoom(colKey)}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, fontWeight: "bold", color: tasks.length === 0 ? "#CCC" : fmr.text, letterSpacing: "0.04em", textTransform: "uppercase" }}>{freq}</span>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
            ...
            <div key={freq} style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: "bold", color: fmr.text, textTransform: "uppercase", letterSpacing: "0.06em" }}>{freq}</span>
                ...
                <button onClick={() => { setShowAddTask(true); setNewTaskFreq(freq); setNewTaskText(""); setNewTaskAssignees([]); setEditingTask(null); }}
                  style={{ fontSize: 11, color: fmr.text, background: fmr.bg, border: `1px solid ${fmr.border}`, borderRadius: 10, padding: "3px 10px", cursor: "pointer", fontFamily: "inherit" }}>
                  + Add
                </button>
              </div>
              ...
              {showAddTask && newTaskFreq === freq && editingTask === null && (
                <div style={{ marginBottom: 8, padding: "10px 12px", background: fmr.bg, border: `1px solid ${fmr.border}`, borderRadius: 10 }}>
                  <input
                    ...
                    style={{ width: "100%", fontSize: 13, padding: "6px 8px", border: `1px solid ${fmr.border}`, borderRadius: 6, fontFamily: "inherit", marginBottom: 8, boxSizing: "border-box" }}
                  />
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 8 }}>
                    {FAMILY.map(m => (
                      <button key={m.id} onClick={() => setNewTaskAssignees(prev => prev.includes(m.id) ? prev.filter(x => x !== m.id) : [...prev, m.id])}
                        style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 12, border: `1px solid ${newTaskAssignees.includes(m.id) ? m.color : "#DDD"}`, background: newTaskAssignees.includes(m.id) ? m.color + "22" : "#fff", cursor: "pointer", fontFamily: "inherit", fontSize: 11 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => {
                      ...
                      setCustomTasks(prev => {
                        ...
                        const existing = snap[editRoom.id][freq];
                        return { ...snap, [editRoom.id]: { ...snap[editRoom.id], [freq]: [...existing, { text: newTaskText.trim(), assignees: newTaskAssignees }] } };
                      });
                      setShowAddTask(false); setNewTaskText(""); setNewTaskAssignees([]);
                    }} style={{ flex: 1, padding: "7px", background: fmr.dot, color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: "bold" }}>
                      Save
                    </button>
                    <button onClick={() => { setShowAddTask(false); setNewTaskText(""); setNewTaskAssignees([]); }}
                      style={{ padding: "7px 14px", background: "#fff", border: "1px solid #DDD", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 12, color: "#888" }}>
                      Cancel
                    </button>
                  </div>
                </div>
                    ...
                    <div style={{ padding: "10px 12px", background: fmr.bg, border: `1px solid ${fmr.border}`, borderRadius: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => {
                          ...
                          setCustomTasks(prev => {
                            const snap = ensureSnapshot(editRoom.id, freq, prev);
                            const updated = [...snap[editRoom.id][freq]];
                            updated[i] = { text: newTaskText.trim(), assignees: newTaskAssignees };
                            return { ...snap, [editRoom.id]: { ...snap[editRoom.id], [freq]: updated } };
                          });
                          setEditingTask(null); setNewTaskText(""); setNewTaskAssignees([]);
                        }} style={{ flex: 1, padding: "7px", background: fmr.dot, color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: "bold" }}>
                          Save
                        </button>
                        <button onClick={() => { setEditingTask(null); setNewTaskText(""); setNewTaskAssignees([]); }}
                          style={{ padding: "7px 14px", background: "#fff", border: "1px solid #DDD", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 12, color: "#888" }}>
                          Cancel
                        </button>
                      </div>
                        ...
                        {task.assignees?.length > 0 && (
                          <div style={{ display: "flex", gap: 3, marginTop: 4 }}>
                            {task.assignees.map(aid => {
                              ...
                              return m ? <Avatar key={aid} member={m} size={16} fontSize={8} /> : null;
                            })}
                          </div>
                        )}
                      </div>
                      <button onClick={() => { setEditingTask({ roomId: editRoom.id, freq, index: i }); setNewTaskText(task.text); setNewTaskAssignees(task.assignees || []); setShowAddTask(false); }}
                        style={{ fontSize: 11, color: "#AAA", background: "none", border: "none", cursor: "pointer", padding: "4px 6px" }}>✏️</button>
                      <button onClick={() => {
                        setCustomTasks(prev => {
                          const snap = ensureSnapshot(editRoom.id, freq, prev);
                          const updated = snap[editRoom.id][freq].filter((_, idx) => idx !== i);
                          return { ...snap, [editRoom.id]: { ...snap[editRoom.id], [freq]: updated } };
                        });
                      }} style={{ fontSize: 11, color: "#D47F6B", background: "none", border: "none", cursor: "pointer", padding: "4px 6px" }}>🗑️</button>
                    </div>
                  )}
                </div>
              ))}
